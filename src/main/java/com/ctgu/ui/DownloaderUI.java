...
public class DownloaderUI extends JFrame
{
  ...
  private void setupCookie()
  {
    ...
    JLabel tip = new JLabel(
        "<html><body>提示：登录CSDN后按F12 -> Network -> 刷新页面 -><br>找到任意请求 -> Request Headers -> 复制Cookie值</body></html>");
  ...
  private void validateUrls()
  {
    ...
    JOptionPane.showMessageDialog(this, message, "URL 有效性验证",
        invalidCount > 0 ? JOptionPane.WARNING_MESSAGE : JOptionPane.INFORMATION_MESSAGE);
  }
  ...
  private void clearTempFiles()
  {
    ...
    int choice =
        JOptionPane.showConfirmDialog(this, "确定要清理下载目录 (" + baseDir.getAbsolutePath() + ") 下的所有文件吗？\n此操作不可恢复！",
            "清理确认", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
      ...
      try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file), StandardCharsets.UTF_8)))
      {
        ...
        for(int i = 0; i < completedModel.getRowCount(); i++)
        {
          ...
          for(int j = 0; j < completedModel.getColumnCount(); j++)
          {
            ...
            line.append(valStr);
            if(j < completedModel.getColumnCount() - 1)
              line.append(",");
          }
          writer.println(line);
        }
  ...
  private void showAboutDialog()
  {
    ...
    // 使用系统默认的消息对话框显示内容
    JOptionPane.showMessageDialog(this, aboutText.toString(), "关于", JOptionPane.INFORMATION_MESSAGE);
  }
