...
public class CSDNDownloader
{
  ...
  private static final String DEFAULT_TEMPLATE =
      "<!DOCTYPE html>" + "<html lang='zh-CN'>" + "<head><meta charset='UTF-8'><title>{{title}}</title>" + "<style>"
          + "  body { font-family: 'PingFang SC', 'Microsoft YaHei', SimHei, sans-serif; line-height: 1.6; padding: 20px; background-color: #f6f8fa; }"
          ...
          + "  * { font-family: 'MyChineseFont', sans-serif !important; }" + "</style>" + "</head>" + "<body>" + "  <div class='paper'>"
          + "    <h1>{{title}}</h1>"
          + "    <div style='color: #888; font-size: 12px; margin-bottom: 20px;'>原文链接: <a href='{{url}}'>{{url}}</a></div>"
  ...
  private void loadConfig()
  {
    ...
    try
    {
      ...
      // 解析 Bot User-Agent
      this.uaBot = props.getProperty("ua.bot", "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)");
  ...
  public DownloadResult downloadArticle(String url)
  {
    try
    {
      ...
      boolean isRestricted =
          doc.select("#content_views").isEmpty() || doc.html().contains("hide-article-box") || title.contains("Custom-Access-Control");
      if(isRestricted)
      {
  ...
  private String processHtml(Document doc, String url)
  {
    ...
    {
      pre.attr("style",
          "white-space: pre-wrap; word-break: break-all; background: #282c34; color: #abb2bf; padding: 10px; border-radius: 5px;");
    }
    String title = doc.title().replace("-CSDN博客", "").trim();
