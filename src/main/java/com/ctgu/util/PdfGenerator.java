...
public class PdfGenerator
{
  ...
  public static void generate(String html, String outputPath) throws Exception
  {
    ...
    // 强制设置为 XHTML 语法
    doc.outputSettings().syntax(Document.OutputSettings.Syntax.xml).escapeMode(org.jsoup.nodes.Entities.EscapeMode.xhtml) // 使用 XHTML 转义模式
        .charset("UTF-8");
    // 2. 转换成 String
    ...
    try (OutputStream os = new FileOutputStream(outputPath))
    {
      ...
      try (InputStream is = PdfGenerator.class.getResourceAsStream("/fonts/Alibaba-PuHuiTi-Regular.ttf"))
      {
        if(is != null)
        {
          ...
          builder.useFont(() -> new ByteArrayInputStream(fontBytes), "MyChineseFont", 400, BaseRendererBuilder.FontStyle.NORMAL, true);
          fontLoaded = true;
        }
      }
      catch(Exception e)
