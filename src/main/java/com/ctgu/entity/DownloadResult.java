...
public class DownloadResult
{
  ...
  public DownloadResult(boolean success, String url, String title, String error, String html, int httpStatus, String contentType,
      long downloadTime, int contentLength, boolean articleExists, String articleId)
  {
    this.success = success;
    this.url = url;
    this.title = title;
  ...
  {
    return DownloadResult.builder().success(false).url(url).error(error).httpStatus(httpStatus).articleExists(httpStatus != 404).build();
  }
}